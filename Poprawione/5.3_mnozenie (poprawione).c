...
void mnozenie(int n, int k, int l, int macierz[n][k][l]){
    int macierz1[n][k],macierz2[n][k];
    jednaMacierz2x2x2NaDwieMacierze2x2(2,2,2,macierz,macierz1,macierz2);
}

void wypisywanieMacierzyWynikowej (int n, int k, int macierz1[n][k], int macierz2[n][k]){
    int i,j,wynikJako2x2[n][k];
    for (i=0;i<n;i++){
...
int main(){
    ...
    mnozenie(2,2,2,macierz3);
    printf("W wyniku mnozenia losowych macierzy powstaje macierz:\n\n");
    wypisywanieMacierzyWynikowej(2,2,macierz1,macierz2);
    printf("\n\n");
    return 0;
}
